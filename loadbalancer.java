...
public class loadbalancer{
	...
	while (true){
		...
		if(clientsentence.contains("lelaki")){
		Socket server = new Socket("192.168.189.129",2345);
		DataOutputStream outtoserver = new DataOutputStream(server.getOutputStream());
		outtoserver.writeBytes(clientsentence+"\n");
		server.close();
		}

		else if(clientsentence.contains("perempuan")){
		Socket server2 = new Socket("192.168.189.130",2346);
